...
2. In Render, choose **New → Blueprint** and select the repository; it reads
   `render.yaml`.
3. Confirm `ALLOWED_ORIGINS` matches the URL Render issues. Leaving it empty
   disables the gateway's Origin check, which lets any site open a WebSocket
   to the IRC server.
4. `IRC_PASSWORD` is generated by Render and is only used between the gateway
   and ft_irc inside the container.

Creating the service through **New → Web Service** instead of a Blueprint skips
`render.yaml` entirely, so no environment variable is applied and the container
stops with `IRC_PASSWORD is not set`. Set the variables by hand in that case.
